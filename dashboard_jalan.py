...
# ========== UI HEADER ==========
st.title("📍 Dashboard Kondisi Jalan Desa - Provinsi Jawa Barat")
...
with tab1:
    ...
    col1.metric("✅ Baik", f"{total_baik:,} m")
    col2.metric("⚠️ Rusak Ringan", f"{rusak_ringan:,} m")
    col3.metric("⚠️ Rusak Sedang", f"{rusak_sedang:,} m")
    col4.metric("❌ Rusak Berat", f"{rusak_berat:,} m")

    st.markdown(f"""
    **DESKRIPSI UMUM:**
    - **Total ruas jalan keseluruhan**: `{total_ruas:,} ruas jalan`
    - **Total panjang jalan keseluruhan**: `{total_jalan:,} meter`
    - Total panjang jalan dalam kondisi **baik**: `{total_baik:,} meter`
    - Total panjang jalan yang mengalami **kerusakan**: `{total_rusak:,} meter`
    """)
    ...
    if {'LAT AWAL', 'LNG AWAL', 'LAT AKHIR', 'LNG AKHIR'}.issubset(df_filtered.columns):
        ...
        if not df_map.empty:
            m = folium.Map(
                ...
                zoom_start=11,
                control_scale=True,
                tiles='CartoDB positron'  # Mirip tampilan Google Maps
            )
            ...
            for _, row in df_map.iterrows():
                ...
                center_lat = (lat_awal + lat_akhir) / 2
                center_lng = (lng_awal + lng_akhir) / 2
                ...
                <a href="{google_maps_link}" target="_blank">📍 Lihat di Google Maps</a>
                """
